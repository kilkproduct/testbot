...
function pickPrefix(guildId) {
  ...
  return store.guilds?.[guildId]?.prefix || GLOBAL_PREFIX;
}
...
function commandPrefixForMessage(message) {
  if (!message.guild) return GLOBAL_PREFIX;
  return pickPrefix(message.guild.id);
}

function findMember(message, token) {
  const id = token?.match(/^<@!?(\d+)>$/)?.[1] || token?.match(/^(\d+)$/)?.[1];
  if (!id) return null;
  return message.guild.members.cache.get(id) || null;
}

function findRole(message, token) {
  const id = token?.match(/^<@&(\d+)>$/)?.[1] || token?.match(/^(\d+)$/)?.[1];
...
function findChannel(message, token) {
  const id = token?.match(/^<#(\d+)>$/)?.[1] || token?.match(/^(\d+)$/)?.[1];
...
async function sendSticky(channel, guildId) {
  const store = readStore();
  const sticky = store.guilds?.[guildId]?.sticky;
  if (!sticky?.channelId || !sticky?.message) return;
  const channelObj = channel.guild.channels.cache.get(sticky.channelId);
  if (!channelObj || channelObj.id !== channel.id) return;

  if (sticky.lastMessageId) {
    try {
      const old = await channel.messages.fetch(sticky.lastMessageId);
...
client.on('messageCreate', async (message) => {
  try {
    ...
    const store = readStore();
    const g = getGuild(store, message.guild.id);

    // autoresponders and sticky messages
    const lower = message.content.toLowerCase();
    const responder = g.autoresponders.find(a => a.trigger.toLowerCase() === lower);
    if (responder) {
      await message.reply(formatTemplate(responder.response, message.member, message.guild)).catch(() => {});
    }

    if (g.sticky?.channelId && g.sticky.channelId === message.channel.id) {
      await sendSticky(message.channel, message.guild.id);
    }
    ...
    switch (command) {
      ...
      case 'help': {
        ...
        return message.reply({
          embeds: [makeEmbed(
            found.name,
            [`Usage: \`${prefix}${found.usage}\``, `Permissions: ${found.perms}`, '', found.description].join('\n')
          )]
        });
      }
      ...
      case 'prefix': {
        ...
        if (sub === 'reset') {
          withStore(data => {
            const g2 = getGuild(data, message.guild.id);
            g2.prefix = null;
          });
      ...
      case 'avatar': {
        const member = findMember(message, args[0]) || message.member;
      ...
      case 'serverinfo': {
        ...
        const embed = makeEmbed(
          g.name,
          [`ID: ${g.id}`, `Members: ${g.memberCount}`, `Owner: <@${g.ownerId}>`, `Created: ${g.createdAt.toDateString()}`].join('\n')
      ...
      case 'roleinfo': {
        ...
        const embed = makeEmbed(
          role.name,
          [`ID: ${role.id}`, `Members: ${role.members.size}`, `Color: ${role.hexColor}`, `Hoisted: ${role.hoist}`, `Mentionable: ${role.mentionable}`].join('\n')
        );
        return message.reply({ embeds: [embed] });
      }

      case 'channelinfo': {
        const channel = findChannel(message, args[0]) || message.channel;
        const embed = makeEmbed(
          `#${channel.name || 'channel'}`,
          [`ID: ${channel.id}`, `Type: ${channel.type}`, `Created: ${channel.createdAt ? channel.createdAt.toDateString() : 'unknown'}`].join('\n')
        );
        return message.reply({ embeds: [embed] });
      }
      ...
      case 'poll': {
        ...
        const embed = makeEmbed('Poll', question);
      ...
      case 'embed': {
        ...
        const embed = makeEmbed(title, description, /^[0-9a-fA-F]{6}$/.test((hex || '').replace('#','')) ? `#${hex.replace('#','')}` : 0x5865F2);
      ...
      case 'slowmode': {
        ...
        const channel = findChannel(message, args[1]) || message.channel;
      ...
      case 'roleremove': {
        const member = findMember(message, args[0]);
        const role = findRole(message, args[1]);
      ...
      case 'goodbye': {
        ...
        if (sub === 'set') {
          ...
          withStore(data => {
            const g = getGuild(data, message.guild.id);
            g[command] = { channelId: channel.id, message: msgText };
          });
      ...
      case 'sticky': {
        ...
        if (sub === 'set') {
          ...
          withStore(data => {
            const g = getGuild(data, message.guild.id);
            g.sticky = { channelId: channel.id, message: stickyText, lastMessageId: null };
          });
    ...
    if (message && message.channel) {
      message.reply('That command failed. Check my permissions and the command syntax.').catch(() => {});
    }
  }
});
...
client.on('guildMemberAdd', async (member) => {
  const store = readStore();
  const g = store.guilds?.[member.guild.id];
